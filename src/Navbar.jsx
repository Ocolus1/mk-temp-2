    ...
    <div className="main-nav-menu-mobile z-50 bg-transparent shadow-sm lg:shadow-none absolute left-0 top-0 bottom-0 right-0 border-r border-slate-400/30 border-dashed w-[270px] md:w-[300px] lg:flex lg:w-[300px] lg:flex-col">
      <button>
        <div className="header-logo p-5 flex items-center space-x-2">
          <h2 className="text-white font-bold text-2xl">Home Services</h2>
        </div>
      </button>
      <div className="header-menu lg:hidden">
        ...
        <div className="header-button space-y-2">
          <div className="px-6 lg:flex lg:items-center">
            <a
              href="#"
              className="flex w-full items-center text-white justify-center rounded-full bg-[#349DBE] px-6 py-2.5 font-semibold hover:shadow-lg hover:drop-shadow transition duration-200"
            >
              <span>Join now</span>
            </a>
          </div>
      ...
      <nav className="z-10 sticky top-0 left-0 right-0 max-w-4xl xl:max-w-5xl mx-auto px-5 py-2.5 lg:border-none lg:py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="header-menu hidden  lg:flex items-center gap-x-10">
            <ul className="flex space-x-10 text-base font-bold text-white/60">
              {menuItems.map((menuItem, index) => (
                <li key={index}>
                  <a href={menuItem.url}>
                    <button>{menuItem.text}</button>
            ...
            <div className="hidden lg:flex lg:items-center">
              <button className="flex items-center text-white justify-center rounded-lg bg-[#45B3BA] px-6 py-2.5 font-semibold hover:shadow-lg hover:drop-shadow transition duration-200">
                <span>Join now</span>
              </button>
            </div>
          </div>
